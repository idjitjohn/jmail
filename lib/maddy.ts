...
export async function listAccounts(): Promise<string[]> {
  const { stdout } = await execFileAsync(SUDO, [MADDY, 'imap-acct', 'list'])
  return stdout
    .split('\n')
    .map(line => line.trim())
...
export async function createAccount(email: string, password: string): Promise<void> {
  ...
  // Create IMAP account
  await execFileAsync(SUDO, [MADDY, 'imap-acct', 'create', safe])

  // Create credentials — maddy creds create reads password from stdin
  await spawnWithStdin(SUDO, [MADDY, 'creds', 'create', safe], password)
}

export async function deleteAccount(email: string): Promise<void> {
  const safe = sanitizeEmail(email)
  await execFileAsync(SUDO, [MADDY, 'imap-acct', 'remove', safe])
  await execFileAsync(SUDO, [MADDY, 'creds', 'remove', safe])
}

export async function resetPassword(email: string, password: string): Promise<void> {
  const safe = sanitizeEmail(email)
  await spawnWithStdin(SUDO, [MADDY, 'creds', 'password', safe], password)
}
